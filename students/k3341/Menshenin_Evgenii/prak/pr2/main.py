...
app = FastAPI(title="Практика 1.2 — воины (ORM)")
...
def seed_if_empty(session: Session) -> None:
    ...
    session.add(SkillWarriorLink(skill_id=s1.id, warrior_id=w1.id))
    session.add(SkillWarriorLink(skill_id=s2.id, warrior_id=w1.id))
    session.commit()


@app.get("/")
def hello() -> str:
    return "Hello, warrior (SQLModel)!"
...
def warriors_get(warrior_id: int, session: Session = Depends(get_session)) -> Warrior:
    ...
    return WarriorRead.model_validate(w)
